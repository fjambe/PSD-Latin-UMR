...
def read_filename(filename):
    """Function to read the input TSV file and convert it to a dataframe. Expected format (8 columns):
    token, token_id, lemma, id_tect, not_constrained_candidates, possible_synsets, wrong_number, wrong_guesses."""
    imported = pd.read_csv(filename, sep='\t', converters={'not_constrained_candidates': pd.eval,
                                                           'possible_synsets': pd.eval})
    return imported
        ...
        for line in infile.readlines():
            ...
            if len(line) == 5:  # standard case
                if line[4].startswith('v#'):  # only verbal frames
                    ...
                    # synset_id may be multiple (separator: /), but it would be the same also in reference corpus
...
def count_verbs(r_verbs, t_verbs):
    """Function that retrieves the most frequent lemmas in the whole corpus. Sum and habeo are excluded."""
    all_lemmas = {**r_verbs, **t_verbs}
    all_lemmas = {k: lm[0] for k, lm in all_lemmas.items() if lm[0] not in ['sum', 'habeo']}
...
def interpret_guesses(imported, exclude=False, lemma_filter=None, seen=None, no_zero_no_one=None):
    """
    Function to compute the average number (arithmetic mean) of candidate guesses extracted before retrieving one with
    the same lemma as the token to be annotated. Options:
    1. exclude cases of lemmas occurring only once (number of guesses = 1645).
    ...
    if lemma_filter:  # lemma_filter is a list of lemmas
        imported = imported[imported['lemma'].isin(lemma_filter)]
    elif seen:
        seen_synsets = [vr[1] for vr in seen.values()]
        to_keep = [k for k, v in tgt_verbs.items() if v[1] in seen_synsets]  # v[1] = synset-XX
        imported = imported[imported['id_tect'].isin(to_keep)]
    elif no_zero_no_one:
        imported = imported[~imported['lemma'].isin(no_zero_no_one)]

    if exclude or no_zero_no_one:
        imported = imported[imported['wrong_number'] != 1645]
        numbers = imported['wrong_number'].tolist()
    else:
        numbers = imported['wrong_number'].tolist()
...
def compare_synsets(imported, annotated, seen):
    """Function to compute the average number (arithmetic mean) of candidate synset extracted before retrieving the
    correct one. No lemma constraint is applied here, as we are considering only lemma-synset pairs that occur in the
    reference corpus. Artificially simplified setup."""
    seen_synsets = [vr[1] for vr in seen.values()]
    ...
    for t_id_tgt, tup_tgt in annotated.items():  # for each entry, i.e. row, in my tgt (annotated) file
        ...
        for index, row in imported.iterrows():
            id_tect = row['id_tect']
            if id_tect == t_id_tgt:
                for cand in row['not_constrained_candidates']:
...
if __name__ == "__main__":
    ...
    oov = len(info[info['wrong_number'] == 1645])  # 54
